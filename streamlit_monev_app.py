...
def create_radar_chart(labels, scores, title, colors, width, height, dpi_val, use_3d=True, use_grad=True, 
    ...
    # Warna grid yang lebih terlihat
    grid_outer_color = '#9ca3af'  # Abu-abu medium untuk grid terluar (level 4)
    grid_inner_color = '#d1d5db'  # Abu-abu muda untuk grid dalam
    spoke_color = '#d1d5db'       # Abu-abu muda untuk garis radial
    
    # Grid konsentris (polygon untuk setiap level) - dari dalam ke luar
    for i in range(1, max_val + 1):
        grid_values = [i] * (num_vars + 1)
        # Grid untuk level 4 (terluar/target) lebih tebal dan lebih gelap
        if i == max_val:
            ax.plot(angles, grid_values, color=grid_outer_color, linewidth=1.8, 
                   linestyle='-', zorder=1, alpha=0.7)
        else:
            ax.plot(angles, grid_values, color=grid_inner_color, linewidth=1.2, 
                   linestyle='-', zorder=1, alpha=0.6)
    
    # Grid radial (garis sumbu dari pusat) - harus terlihat jelas
    for angle in angles[:-1]:
        ax.plot([angle, angle], [0, max_val], color=spoke_color, linewidth=1.2, 
               zorder=1, alpha=0.6)
